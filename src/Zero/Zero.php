...
class Zero
{
    ...
    public function getConversionInfo($user, $bought)
    {
        ...
        if (isset($shopinfo['traffic_package'])) {  /* 是流量包 */
            ...
            # 折算模式
            $mode = ZeroConfig::get('shop_conversion_mode');
            if ($mode=='auto') {
        ...
        /* 手续费比例 */
        $hdpr = ZeroConfig::get('shop_formalities');
        /* 手续费金额 */
    ...
    /**
     * 使用 ZeroPay 聚合支付时, 商店购买套餐支付回调后自动购买套餐函数
     */
    public static function zeropay_buyshop($pid=0)
    {
        $ps            = Order::where('tradeno', $pid)->first();
        ...
        $shop          = Product::where('id', $shop)->where('status', 1)->first();
    ...
    public static function getNodeFilter($token=null, $user=null)
    {
        if ($token) {
            ...
            $link = Link::where('userid', $user->id)->first();
        }
