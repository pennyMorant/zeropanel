        ...
        if ($user != null) {
            switch (true) {
                case (strpos($callback_data, 'mu')):
                    $ssr_sub_token = LinkController::GenerateSubCode($user->id);
                    $subUrl = Setting::obtain('subscribe_address_url') . '/link/';
...
未使用 ' . $user->unusedTraffic() . ' ' . number_format(($user->transfer_enable - ($user->u + $user->d)) / $user->transfer_enable * 100, 2) . '%';
        ...
        if ($message->getChat()->getId() > 0) {
            ...
            switch ($command) {
                ...
                default:
                    ...
                    foreach ($photo_id_array as $key => $value) {
                        ...
                        $qrcode_text = QRcode::decode('https://api.telegram.org/file/bot' . Setting::obtain('telegram_bot_token') . '/' . $file->getFilePath());

                        if ($qrcode_text == null) {
                            foreach ($photo_id_list_array[$key] as $fail_key => $fail_value) {
                                $fail_file = $bot->getFile($fail_value);
                                $qrcode_text = QRcode::decode('https://api.telegram.org/file/bot' . Setting::obtain('telegram_bot_token') . '/' . $fail_file->getFilePath());
                                if ($qrcode_text != null) {
                                    break;
                                }
                            }
            ...
            switch ($command) {
                ...
                default:
                    ...
                    if ($message->getNewChatMember() != null && Setting::obtain('enable_welcome_message_telegram_notify') == true) {
    ...
    {
        try {
            $bot = new Client(Setting::obtain('telegram_bot_token'));
